...
void loop(){
      ...
      if(!header_finished){
        ...
        if (header.indexOf("POST")>=0){
          if (contentLength != 0)  {
            ...
            */
            if(header.indexOf("Content-Type: application/x-shrimp")>=0){
              ...
              while(bodyLengthCounter < contentLength){
                ...
                int shrimpStatus = shrimpCMD(command, epaper, widgetMaster);
              }
              switch (shrimpStatus){
                case 1:
  ...
  // checks for time differentials since last loop and then may trigger other events
